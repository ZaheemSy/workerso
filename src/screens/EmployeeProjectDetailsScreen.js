  ...
  const renderLog = ({ item }) => (
    <View style={styles.logRow}>
      ...
      <View style={styles.logTextWrap}>
        ...
        <Text style={styles.logDate}>Log date: {item.date || new Date(item.createdAt).toISOString().split('T')[0]}</Text>
        <Text style={styles.logDate}>Entered: {formatDateTime(item.createdAt)}</Text>
        {item.updatedAt && item.updatedAt !== item.createdAt ? (
          <Text style={styles.logDate}>Edited: {formatDateTime(item.updatedAt)}</Text>
        ) : null}
      </View>
      {canAddLog ? (
