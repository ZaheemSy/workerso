...
const QuickProjectGalleryScreen = ({ navigation, route }) => {
  ...
  const shareImageItem = useCallback(async item => {
    ...
    try {
      ...
      const lines = [`Project: ${project?.name || 'Quick Project'}`];
      if (item.description?.trim()) {
        lines.push(`Description: ${item.description.trim()}`);
      }
      if (taggedNames.length > 0) {
        lines.push(`Tagged: ${taggedNames.join(', ')}`);
      }
  ...
  }, [getTaggedEmployees, project?.name, toDisplayUri]);

  const loadData = useCallback(async () => {
    if (!session?.orgId || !quickProjectId) return;
    setLoading(true);
    try {
      const [projectData, employeeList] = await Promise.all([
        getQuickProjectById(quickProjectId),
        getQuickEmployeesByOrg(session.orgId),
      ]);
      if (!projectData) {
        Alert.alert('Not Found', 'Quick project not found.');
        navigation.goBack();
        return;
      }
      setProject(projectData);
      setEmployees(employeeList.sort((a, b) => (a.name || '').localeCompare(b.name || '')));
      const items = Array.isArray(projectData.quickGalleryItems) ? projectData.quickGalleryItems : [];
      setGalleryItems(items.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')));
    } catch (error) {
      Alert.alert('Error', 'Failed to load quick gallery');
    } finally {
      setLoading(false);
    }
  }, [navigation, quickProjectId, session?.orgId]);
  ...
  const persistItems = useCallback(async nextItems => {
    const saved = nextItems.slice().sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));
    await updateQuickProject(quickProjectId, { quickGalleryItems: saved });
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      try {
        ...
        // Fallback path for devices/emulators with limited camera capabilities.
      ...
      setCameraReady(false);
      setShowCamera(false);
      setCapturedPhotoPath(newItem.imageUri);
  ...
  const saveCapturedItem = async () => {
    ...
    const newItem = {
      quickGalleryId: generateId('quick_gallery'),
  ...
  const selectedDetailItem = useMemo(
    () => galleryItems.find(item => item.quickGalleryId === selectedDetailId) || null,
    [galleryItems, selectedDetailId]
  );

  const openDescriptionModal = item => {
    setEditingDescriptionId(item.quickGalleryId);
  ...
  const saveDescription = async () => {
    const next = galleryItems.map(item => (
      item.quickGalleryId === editingDescriptionId
  ...
  const openTagModal = item => {
    setEditingTagId(item.quickGalleryId);
    setDraftTagEmployeeIds(item.taggedEmployeeIds || []);
    setTagSearchQuery('');
    setShowTagModal(true);
  };

  const toggleDraftTag = quickEmployeeId => {
  ...
  const saveTags = async () => {
    const next = galleryItems.map(item => (
      item.quickGalleryId === editingTagId
  ...
  const employeesForTagModal = useMemo(() => {
  ...
  const toggleCapturedTag = quickEmployeeId => {
  ...
  const renderGalleryCard = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          setSelectedDetailId(item.quickGalleryId);
      ...
      >
        ...
        <View style={styles.listInfo}>
          ...
          <Text style={[styles.listDescription, !item.description && styles.descriptionPlaceholder]} numberOfLines={1}>
  ...
  const renderTagChips = item => {
    ...
    return (
      <View style={styles.tagsWrap}>
        {tagged.map(emp => (
          <View key={emp.quickEmployeeId} style={styles.tagChip}>
    ...
    return (
      <View style={styles.cameraContainer}>
        <View style={styles.cameraHeader}>
          ...
          <Text style={styles.cameraHeaderTitle}>Quick Gallery Camera</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Quick Project Gallery</Text>
        <View style={styles.iconSpacer} />
      </View>

      <View style={styles.content}>
        <View style={styles.infoCard}>
          <Text style={styles.infoTitle}>{project?.name || 'Quick Project'}</Text>
        ...
        {loading ? (
          ...
          <FlatList
            data={filteredItems}
            keyExtractor={item => item.quickGalleryId}
      ...
      <Modal
        transparent
        visible={showCapturePreviewModal}
        animationType="fade"
        onRequestClose={cancelCapturedItem}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={[styles.previewDescription, !capturedDescriptionDraft && styles.descriptionPlaceholder]}>
          ...
          <View style={styles.modalCard}>
            ...
            {selectedDetailItem ? (
              <>
                ...
                <Text style={[styles.previewDescription, !selectedDetailItem.description && styles.descriptionPlaceholder]}>
      ...
      <Modal
        transparent
        visible={showCaptureDescriptionModal}
        animationType="fade"
        onRequestClose={() => setShowCaptureDescriptionModal(false)}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                const selected = capturedTagEmployeeIds.includes(item.quickEmployeeId);
                return (
                  <TouchableOpacity style={styles.selectRow} onPress={() => toggleCapturedTag(item.quickEmployeeId)}>
      ...
      <Modal transparent visible={showTagModal} animationType="fade" onRequestClose={() => setShowTagModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <FlatList
              data={employeesForTagModal}
              keyExtractor={item => item.quickEmployeeId}
              renderItem={({ item }) => {
                const selected = draftTagEmployeeIds.includes(item.quickEmployeeId);
                return (
                  <TouchableOpacity style={styles.selectRow} onPress={() => toggleDraftTag(item.quickEmployeeId)}>
                    {selected ? <CheckSquare color={COLORS.primary} size={18} /> : <Square color={COLORS.gray} size={18} />}
                    <Text style={styles.selectRowText}>{item.name}</Text>
                  </TouchableOpacity>
                );
      ...
      <Modal transparent visible={showFilterModal} animationType="fade" onRequestClose={() => setShowFilterModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <FlatList
              data={employeesForFilterModal}
              keyExtractor={item => item.quickEmployeeId}
              renderItem={({ item }) => {
                const selected = draftFilterEmployeeIds.includes(item.quickEmployeeId);
                return (
                  <TouchableOpacity style={styles.selectRow} onPress={() => toggleDraftFilter(item.quickEmployeeId)}>
                    {selected ? <CheckSquare color={COLORS.primary} size={18} /> : <Square color={COLORS.gray} size={18} />}
                    <Text style={styles.selectRowText}>{item.name}</Text>
                  </TouchableOpacity>
                );
...
const styles = StyleSheet.create({
  ...
  cardImage: { width: '100%', height: 200, backgroundColor: COLORS.extraLightGray },
  cardBody: { padding: 12 },
  dateText: { color: COLORS.textLight, fontSize: 11 },
  descriptionText: { marginTop: 6, color: COLORS.text, fontSize: 13, lineHeight: 18 },
  descriptionPlaceholder: { color: COLORS.gray },
...
export default QuickProjectGalleryScreen;
